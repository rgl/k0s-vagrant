...
CONFIG_K0SCTL_VERSION = 'v0.16.0' # see https://github.com/k0sproject/k0sctl/releases
CONFIG_K0S_VERSION = 'v1.28.4+k0s.0' # see https://github.com/k0sproject/k0s/releases
CONFIG_KUBECTL_VERSION = '1.28.2' # CONFIG_K0S_VERSION.gsub(/v([\.\d]+).+/, '\1')
CONFIG_K9S_VERSION = 'v0.28.2' # see https://github.com/derailed/k9s/releases
CONFIG_KREW_VERSION = 'v0.4.4' # see https://github.com/kubernetes-sigs/krew/releases
CONFIG_ETCDCTL_VERSION = 'v3.5.10' # see https://github.com/etcd-io/etcd/releases BUT make sure you use a version compatible with k0s go.mod.
CONFIG_HELM_VERSION = 'v3.13.2' # see https://github.com/helm/helm/releases BUT make sure you use a version compatible with k0s go.mod.
CONFIG_HELMFILE_VERSION = '0.159.0' # see https://github.com/helmfile/helmfile/releases

# see https://launchpad.net/ubuntu/+archivemirrors
# see https://launchpad.net/ubuntu/+mirror/mirrors.up.pt-archive
# CONFIG_PACKAGE_MIRROR = 'http://mirrors.up.pt/ubuntu/'
CONFIG_PACKAGE_MIRROR = 'http://mirrors.up.pt/debian/'
...
#  config.vm.box = 'boxen/ubuntu-22.04'
  ...
  config.vm.define :pandora do |config|
    ...
    config.vm.provision 'shell', path: 'provision-apt-cacher.sh', args: [CONFIG_PACKAGE_MIRROR, CONFIG_PANDORA_FQDN]
    config.vm.provision 'shell', path: 'provision-base.sh', args: [CONFIG_PACKAGE_MIRROR, CONFIG_PANDORA_FQDN, CONFIG_PANDORA_IP_ADDRESS]
